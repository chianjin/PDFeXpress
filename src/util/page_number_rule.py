...
def _parse_phys_range(expr: str, total: int) -> list[int]:
    ...
    if '-' in expr:
        ...
        if lo < 1 or hi > total or lo > hi:
            raise ValueError(f'Invalid page range: {expr!r}')
        return list(range(lo, hi + 1))
    page = int(expr)
    if page < 1 or page > total:
        raise ValueError(f'Invalid page number: {expr!r}')
    return [page]
...
def _format_number(value: int, type_char: str) -> str:
    ...
    raise ValueError(f'Unknown number type: {type_char!r}')
...
def _parse_display_format(fmt: str):
    ...
    try:
        ...
        raise ValueError(f'Invalid start value in format: {fmt!r}') from None
