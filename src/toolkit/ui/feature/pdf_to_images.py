...
class PDFToImagesApp(ttk.Frame, TaskRunnerMixin):
    def __init__(self, master, **kwargs):
        ...
        ttk.Label(self.option_frame, text=_("Format:")).pack(
            side="left", padx=(10, 5), pady=5
        )
        self.format_var = tk.StringVar(value="jpg")
        self.format_var.trace_add("write", self._on_format_changed)
        ttk.Radiobutton(
            self.option_frame, text="PNG", variable=self.format_var, value="png"
        ).pack(side="left", padx=5, pady=5)
        ttk.Radiobutton(
            self.option_frame, text="JPG", variable=self.format_var, value="jpg"
        ).pack(side="left", padx=5, pady=5)
