...
class AddPageNumbers(ttk.Frame, TaskRunnerMixin):
    ...
    def create_widgets(self):
        TitleFrame(self, _('Add Page Numbers')).grid(
        ...
        self.input_file_picker.grid(
            row=1, column=0, sticky='nsew', padx=10, pady=(0, 5)
        )

        self.output_file_picker = FilePicker(
            self, title=_('Output PDF'), file_types=FILE_TYPES_PDF, mode='save'
        )
        ...
        ttk.Label(rule_option, text=_('Page Number Format:')).grid(
        ...
        ttk.Label(
            rule_option, text=_(PAGE_NUMBERING_FORMAT['brief']), wraplength=600
        ...
        ttk.Label(pos_option, text=_('Position:')).grid(
            row=0, column=0, sticky='w', padx=5, pady=5
        )
        ...
        ttk.Separator(pos_option, orient='vertical').grid(
            row=0, column=3, sticky='ns', padx=20
        )
        ...
        ttk.Label(font_option, text=_('Font:')).grid(
            row=0, column=0, sticky='w', padx=5, pady=5
        )
        ...
        ttk.Label(font_option, text=_('Style:')).grid(
        ...
        ttk.Label(font_option, text=_('Size:')).grid(
        ...
        self.v_margin_spin = ttk.Spinbox(
            margin_option, from_=0, to=10, increment=0.1, format='%.1f'
        )
        ...
        self.h_margin_spin = ttk.Spinbox(
            margin_option, from_=0, to=10, increment=0.1, format='%.1f'
        )
        ...
        self.run_button = ttk.Button(
            bottom_frame, text=_('Add'), command=self.run_task_from_ui
        )
    ...
    def _prepare_task(self):
        ...
        if not rule:
            messagebox.showerror(
                _('Error'), _('Page number format rule cannot be empty.')
            )
            return None
        ...
        try:
            ...
            messagebox.showerror(
                _('Error'), _('Please enter valid numbers for font size and margins.')
            )
            return None
