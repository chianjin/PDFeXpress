...
class ExecuteFrame(ttk.LabelFrame):
    ...
    def _setup_ui(self):
        ...
        self.cancel_button = ttk.Button(
            execute_frame, text='取消', command=self._on_cancel
        )
        self.cancel_button.grid(row=0, column=2, padx=(5, 0))

        # 关闭按钮
        self.close_button = ttk.Button(
            execute_frame, text='关闭', command=self._on_close
        )
    ...
    def set_execute_handler(self, handler):
        ...
        Args:
            handler: 返回 (feature_id, params) 的函数
        """
    ...
    def _check_queue(self):
        ...
        # 检查进程是否存活（仅在未收到完成消息时）
        if (
            self.task_manager._worker_process
            and not self.task_manager._worker_process.is_alive()
        ):
            if self.task_manager.is_running:
                self._handle_message(
                    CompleteMessage(success=False, error=RuntimeError('进程意外终止'))
                )
            return
    ...
    def _handle_status(self, state: TaskState, message: str):
        ...
        if state != self._current_state:
            print(
                f'[DEBUG] State changed from {self._current_state.value if self._current_state else "None"} to {state.value}'
            )
    ...
    def _handle_complete(
        self, success: bool, cancelled: bool = False, error: Exception | None = None
    ):
        """处理完成消息"""
        self.progress_bar.stop()
